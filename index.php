...
define('COMPANYNAME', 'Canadian Immigration and Legal Services Inc.');
define('LIVE_DOMAIN', 'canadianimmigrationlegal.com');

$approvedDomains = array(LIVE_DOMAIN, 'skitsbd.com','canadianimmigrationlegall.com');
...
if(in_array($subdomain, array('', 'www'))){
	$title = 'Canadian Immigration & Legal Services Inc';
	...
	if(in_array($segment2URI, array('appointments', 'about-us', 'home', 'forbidden', 'our-team', 'immigration', 'fingerprint', 'legal-services', 'fingerprint-services', 'immigration-assessment', 'immigration-services', 'videos-main', 'gallery-main', 'search', 'sendAppointments', 'contact-us', 'why-choose-us', 'news-articles', 'sendContactUs','fetchNews'))){
		
		$functionName = str_replace('contact-us', 'contactUs', $segment2URI);
		$functionName = str_replace('home', 'home', $functionName);
		$functionName = str_replace('immigration-assessment', 'immigrationAssessment', $functionName);
		...
		$functionName = str_replace('immigration-services', 'immigrationServices', $functionName);
		...
		elseif($functionName=='immigrationAssessment'){			
			$viewFunctions[$functionName] = ucwords(strtolower("Immigration Assessment"));
			$title = ucwords(strtolower("Immigration Assessment"));
		}
		...
		elseif($functionName=='immigrationServices'){
			$title = trim(stripslashes('Immigration Services'));
		}
	...
	elseif(!empty($segment2URI)){ 
		...
		if(!$tableObj){
			$tableObj = $db->getObj("SELECT customers_id AS id, name, 'teams' AS tableName FROM customers WHERE uri_value = :uri_value", array('uri_value'=>$segment2URI));
		}
...
if(!in_array($segment1, array('Home', 'Appointments', 'Customers', 'News_articles', 'Services', 'Activity_Feed', 'Manage_Data', 'Settings', 'Common'))){	
	if(isset($_SESSION["users_id"])){
...
elseif($segment1=='Customers'){$viewFunctions = array('lists'=>stripslashes('Manage Team'), 'view'=>stripslashes("Team Information"));}
